...
function translationJoinLimit(articleCount: number): number {
	return Math.min(
		...
		Math.max(articleCount * TRANSLATIONS_PER_ARTICLE_HEADROOM, TRANSLATION_JOIN_MIN)
	);
}
...
async function fetchTranslationsForArticleIds(articleIds: string[]): Promise<ArticleTranslation[]> {
	...
	for (let i = 0; i < articleIds.length; i += ARTICLE_ID_CHUNK_SIZE) {
		...
		const res = await databases.listDocuments(DATABASE_ID, COLLECTIONS.ARTICLES_TRANSLATIONS, [
