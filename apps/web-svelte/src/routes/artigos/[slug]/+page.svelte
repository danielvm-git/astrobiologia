<script lang="ts">
  ...
  const articleUrl = $derived(
    `https://astrobiologia.com.br/${lang}/artigos/${slug}`
  );
  ...
  const schemaJson = $derived(
    JSON.stringify(schemaMarkup).replace(/</g, "\\u003c")
  );
...
<svelte:head>
  ...
  <meta
    ...
    content={article.authorName || "Danilo Albergaria"}
  />
  <meta
    property="article:section"
    content={copy.categoryLabel(article.category)}
  />
...
<main class="min-h-screen bg-background">
  <!-- Page Header -->
  <div
    class="relative overflow-hidden pt-32 pb-16 md:pt-48 md:pb-24 border-b border-slate-100 bg-white"
  >
    <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
      {#if category || showMissingTranslationBadge}
        <div class="mb-8 flex flex-wrap items-center gap-3">
          {#if category}
            <a
              href="/{lang}/categorias/{article.category}"
              class="inline-block text-primary text-xs font-black uppercase tracking-[0.2em] hover:underline"
            >
          ...
          {#if showMissingTranslationBadge}
            <span
              class="inline-flex rounded-full bg-amber-100 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-amber-950 ring-1 ring-amber-200"
              role="status"
            >
      ...
      <h1
        class="text-4xl md:text-6xl lg:text-7xl font-black text-slate-900 leading-[1.05] mb-10 text-balance font-sans tracking-tight"
      >
        {title}
      </h1>

      <p
        class="text-xl md:text-2xl text-slate-600 leading-relaxed mb-12 font-serif text-balance italic"
      >
        {excerpt}
      </p>

      <div
        class="flex flex-wrap items-center gap-8 text-slate-400 text-[10px] font-black uppercase tracking-widest border-t border-slate-100 pt-8"
      >
        <div class="flex items-center gap-3">
          ...
          <div class="text-slate-900">
            {article.authorName || "Danilo Albergaria"}
          </div>
        </div>
    ...
    <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-12">
      <figure>
        ...
        {#if article.featuredImageAlt}
          <figcaption
            class="mt-4 text-center text-slate-500 text-sm italic font-serif"
          >
  ...
  <article class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-12 md:py-24">
    ...
    <!-- Footer Meta -->
    <div
      class="mt-24 pt-12 border-t border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-8"
    >
      {#if article.tags && article.tags.length > 0}
        <div class="flex flex-wrap gap-3">
          {#each article.tags as tag}
            <span
              class="px-3 py-1 bg-slate-100 text-slate-600 text-[10px] font-bold uppercase tracking-widest"
            >
              #{tag}
            </span>
          {/each}
  ...
  {#if relatedArticles && relatedArticles.length > 0}
    <section class="bg-slate-50 border-t border-slate-100 py-24">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <h2
          class="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 mb-16 text-center"
        >
          {copy.continue_exploring}
        </h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-12">
          {#each relatedArticles as related}
            {@const rTitle =
              related.translation?.title || (related as any).title}
            {@const rSlug = related.translation?.slug || (related as any).slug}
            {@const rExcerpt =
              related.translation?.excerpt || (related as any).excerpt}
            <div
              class="flex flex-col h-full bg-white border border-slate-100 hover:shadow-xl transition-all duration-300"
            >
              <a
              ...
              >
                {#if related.featuredImage}
                  <img
                    src={related.featuredImage.startsWith("http")
                      ...
                      : `${getImageUrl(related.featuredImage, 600, 375)}&output=webp&quality=80`}
                    alt={rTitle}
                ...
                {:else}
                  <div
                    class="w-full h-full flex items-center justify-center text-4xl bg-slate-50 opacity-30 grayscale"
                  >
                    🔭
                  </div>
                {/if}
              ...
              <div class="p-6 flex flex-col flex-1">
                ...
                >
                  <h3
                    class="text-xl font-bold text-slate-900 mb-4 line-clamp-2 font-serif leading-tight"
                  >
                    {rTitle}
                  </h3>
                </a>
                <p
                  class="text-slate-500 text-sm line-clamp-2 font-serif mb-6 italic"
                >
                  {rExcerpt}
                </p>
                <div
                  class="mt-auto pt-6 border-t border-slate-50 text-[10px] font-black uppercase tracking-widest text-slate-400"
                >
