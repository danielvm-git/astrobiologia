...
export const actions: Actions = {
    save: async (event) => {
        ...
        try {
            ...
            logger.debug('Saving article', { id: params.id, translationsCount: translations.length });
            ...
            logger.debug('Existing translations found', { count: existingTrans.total });

            // 3. Upsert translations
            for (const trans of translations) {
                const existing = existingTrans.documents.find((t: any) => t.language === trans.language);
