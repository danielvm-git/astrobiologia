...
import { DEFAULT_REQUEST_LANGUAGE, getRequestLanguage } from '../../src/lib/server/request-language';
...
describe('getRequestLanguage', () => {
	...
	it('returns default when lang is empty string', () => {
		expect(getRequestLanguage({ paraglide: { lang: '' } })).toBe(DEFAULT_REQUEST_LANGUAGE);
	});
