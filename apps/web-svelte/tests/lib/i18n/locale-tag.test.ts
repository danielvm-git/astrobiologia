...
import { localeTagsMatch, normalizeLocaleTag, primaryLanguageSubtag } from '../../../src/lib/i18n/locale-tag';
...
describe('shouldShowTranslationFallbackBadge', () => {
	...
	it('shows badge when UI is es but translation row is en', () => {
		expect(shouldShowTranslationFallbackBadge('es', { language: 'en' })).toBe(true);
	});

	it('hides badge when translation matches UI locale', () => {
		expect(shouldShowTranslationFallbackBadge('es', { language: 'es-419' })).toBe(false);
	});
});
