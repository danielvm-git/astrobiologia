    ...
    <template v-if="!isLoginPage">
      ...
      >
        ...
        >
          ...
          >
            ...
            <span
              v-if="isSidebarOpen"
              class="font-black text-lg tracking-tighter text-white uppercase truncate"
              >Astro Admin</span
            >
          </NuxtLink>
        </div>
        ...
        <nav class="flex-1 py-6 px-3 space-y-1 overflow-y-auto">
          ...
          >
            ...
            <div
              v-if="!isSidebarOpen"
              class="absolute left-full ml-4 px-2 py-1 bg-slate-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50 whitespace-nowrap"
            >
              {{ item.name }}
            </div>
          </NuxtLink>
        ...
        <div class="p-4 border-t border-slate-800 space-y-2">
          <div
            v-if="isSidebarOpen && user"
            class="flex items-center gap-3 px-2 py-3 bg-slate-800/50 rounded-xl mb-4"
          >
            <div
            ...
            <div class="min-w-0">
              <p
                class="text-xs font-black text-white truncate uppercase tracking-widest"
              >
                {{ user.name || "Admin" }}
              </p>
          ...
          >
            <X v-if="isSidebarOpen" class="w-4 h-4" />
      ...
      >
        ...
        >
          ...
          <span class="font-black text-sm tracking-tighter uppercase"
            >Astro Admin</span
          >
        </NuxtLink>
        <button
      ...
      <div class="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        >
          <div class="flex items-center gap-4">
            ...
            >
            <span
              class="px-2 py-0.5 bg-green-100 text-green-700 text-[9px] font-black uppercase tracking-widest rounded border border-green-200"
              >Produção</span
            >
          </div>
          ...
          <div class="flex items-center gap-6">
            <div
              class="flex items-center gap-2 px-3 py-1.5 bg-slate-100 rounded-full border border-slate-200"
            >
              <div
      ...
      <template v-if="isMobileOpen">
        ...
        <!-- Sidebar -->
        <div
          class="fixed inset-y-0 left-0 w-72 bg-slate-900 flex flex-col z-[70] md:hidden shadow-2xl animate-in slide-in-from-left duration-300"
        >
          <div
            class="h-16 flex items-center justify-between px-6 border-b border-slate-800"
          >
            <span class="font-black text-white uppercase tracking-widest"
              >Menu</span
            >
            <button @click="isMobileOpen = false" class="text-slate-400 p-2">
              <X class="w-6 h-6" />
            </button>
          </div>
          ...
          <div class="p-6 border-t border-slate-800">
            <button
              @click="handleLogout"
              class="flex items-center gap-3 text-red-400 font-bold uppercase tracking-widest text-xs w-full"
            >
