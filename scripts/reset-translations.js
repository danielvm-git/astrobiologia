...
async function resetAndMigrate() {
    ...
    const transResponse = await databases.listDocuments(DATABASE_ID, TRANSLATIONS_COLLECTION_ID, [Query.limit(100)]);
    for (const doc of transResponse.documents) {
        await databases.deleteDocument(DATABASE_ID, TRANSLATIONS_COLLECTION_ID, doc.$id);
    ...
    for (const article of articles) {
        console.log(`Creating PT translation for: ${article.title} (${article.$id})`);
