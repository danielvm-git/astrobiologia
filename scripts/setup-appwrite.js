...
async function setup() {
  ...
  try {
    ...
    if (e.code === 409 || (e.message && e.message.includes('maximum number of databases'))) {
      console.log('⚠️  Database already exists, continuing with existing database...');
    } else {
      throw e;
    }
  }
  ...
  for (const attr of stringAttrs) {
    try {
      await databases.createStringAttribute(DATABASE_ID, ARTICLES_COLLECTION_ID, attr.key, attr.size, attr.required);
  ...
  // Tags array
  try {
    await databases.createStringAttribute(DATABASE_ID, ARTICLES_COLLECTION_ID, 'tags', 100, false, undefined, true);
  ...
  // Status enum
  try {
    await databases.createEnumAttribute(DATABASE_ID, ARTICLES_COLLECTION_ID, 'status', ['draft', 'published'], true);
  ...
  // Featured boolean
  try {
    await databases.createBooleanAttribute(DATABASE_ID, ARTICLES_COLLECTION_ID, 'featured', true);
  ...
  try {
    await databases.createDatetimeAttribute(DATABASE_ID, ARTICLES_COLLECTION_ID, 'publishedAt', false);
  ...
  console.log('\n🔍 Creating indexes...');
  try {
    await databases.createIndex(DATABASE_ID, ARTICLES_COLLECTION_ID, 'slug_index', 'unique', ['slug']);
  ...
  await delay(300);
  try {
    await databases.createIndex(DATABASE_ID, ARTICLES_COLLECTION_ID, 'status_index', 'key', ['status']);
...
async function createAdminUser() {
  ...
  if (!adminEmail || !adminPassword) {
    console.log('⚠️  Skipping user creation: APPWRITE_ADMIN_EMAIL or APPWRITE_ADMIN_PASSWORD not set in .env');
    return;
  }

  try {
    await users.create(ID.unique(), adminEmail, undefined, adminPassword, 'Admin');
