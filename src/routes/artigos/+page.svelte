<script lang="ts">
	...
	const filteredArticles = $derived(
		data.articles.filter((article: Article) => {
			...
			const blob = `${a.translation?.excerpt ?? ''} ${a.translation?.content ?? ''} ${a.description ?? ''} ${a.excerpt ?? ''}`
				.toLowerCase();
...
<svelte:head>
	...
	<meta property="og:title" content={`${m.page_artigos_heading()} — ${m.site_title()}`} />
...
<main class="min-h-screen bg-background">
	...
	<section class="border-t border-slate-200 bg-slate-50 py-12 md:py-16">
		<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
			<div class="mb-12 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
