<script lang="ts">
  ...
  const recentArticles = $derived(
    (data.recent || []).filter(
      (a: any) => activeCategory === "all" || a.category === activeCategory
...
<main class="bg-background min-h-screen">
  ...
  <header class="bg-slate-900">
    <div
      class="mx-auto max-w-7xl px-4 pt-6 pb-3 text-center sm:px-6 md:pt-7 md:pb-5 lg:px-8"
    >
      <h1
        class="mb-1 font-sans text-4xl font-black tracking-tight text-white md:mb-2 md:text-6xl"
      >
        {copy.hero_title}<span class="text-accent">.</span>
      </h1>
      <p
        class="mx-auto max-w-2xl font-serif text-lg italic text-slate-400 md:text-xl"
      >
        {copy.hero_subtitle}
      </p>
    </div>
  ...
  <section class="py-12 md:py-16">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div
        class="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12 border-b border-slate-200 pb-8"
      >
        <div>
          <h2
            class="text-2xl font-bold text-slate-900 font-sans uppercase tracking-widest"
          >
            {copy.section_latest}
          </h2>
        </div>
        ...
        <div class="flex flex-wrap gap-2">
          <button
            class="rounded-full px-5 py-2 text-sm font-semibold transition-all {activeCategory ===
            'all'
      ...
      {#if !data.listLoadError && recentArticles.length > 0}
      ...
      {:else if !data.listLoadError}
        <div
          class="py-32 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-300"
        >
  ...
  <section class="py-20 md:py-32 bg-slate-50 border-t border-slate-200">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div class="max-w-4xl mx-auto text-center">
        <h2
          class="text-3xl md:text-4xl font-black text-slate-900 mb-6 font-sans tracking-tight"
        >
