...
test.describe('[P0] Admin login → dashboard redirect', () => {
	test('lands on dashboard after successful email/password login', async ({ page }) => {
	...
	test('stays on login page after failed login', async ({ page }) => {
		...
		await page.getByPlaceholder('E-mail Administrativo').fill('hacker@example.com');
		...
		await expect(page.locator('text=/Login falhou|credenciais/i')).toBeVisible({ timeout: 5_000 });
	});

	test('[P1] already-authenticated user visiting /admin/login is redirected to dashboard', async ({ page, context }) => {
