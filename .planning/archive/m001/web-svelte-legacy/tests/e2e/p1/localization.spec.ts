...
test.describe("@p1 Localization", () => {
  ...
  for (const lang of languages) {
    test(`should translate scaffold to ${lang.name} (${lang.code})`, async ({
      page,
    }) => {
      ...
      await expect(
        page.getByRole("link", { name: lang.home, exact: true }).first()
      ).toBeVisible();
      await expect(
        page.getByRole("link", { name: lang.about, exact: true }).first()
      ).toBeVisible();

      // Verify Hero Title (if on homepage)
      await expect(
        page.getByRole("heading", { level: 1, name: lang.hero })
      ).toBeVisible();
    });
  }
